 ...
 */
public class STItem2 extends LinearLayout {
    ...
    private int rectAroundCorners;

    private int defaultMarginTop;

    private boolean enabledTouch;

    public STItem2(@NonNull Context context) {
        super(context);
        init(context, null);
    }

    public STItem2(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public STItem2(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    protected void init(@NonNull Context context, AttributeSet attrs) {
         ...
         * 设置项图标大小（长和宽保持一致）
         */
        int iconSize = context.getResources().getDimensionPixelSize(R.dimen.st_item2_icon_size);
        /**
         * 设置项向右图标大小（长和宽保持一致）
         */
        ...
        if (attrs != null) {
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.STItem2);
            title = a.getString(R.styleable.STItem2_STItem2_title);
            titleSize = (a.getDimensionPixelSize(R.styleable.STItem2_STItem2_titleSize, context.getResources().getDimensionPixelSize(R.dimen.text_size_secondary_title)));
            desc = a.getString(R.styleable.STItem2_STItem2_desc);
            layoutPosition = a.getInt(R.styleable.STItem2_STItem2_layoutPosition, POSITION_MIDDLE);
            hasNext = a.getBoolean(R.styleable.STItem2_STItem2_hasNext, true);
            iconResId = a.getResourceId(R.styleable.STItem2_STItem2_icon, 0);
            isLayout = a.getBoolean(R.styleable.STItem2_STItem2_isLayout, false);
            titleWrap = a.getBoolean(R.styleable.STItem2_STItem2_titleWrap, false);
            enabledTouch = a.getBoolean(R.styleable.STItem2_STItem2_enabledTouch, true);
            defaultMarginTop = a.getDimensionPixelSize(R.styleable.STItem2_STItem2_defaultMarginTop, context.getResources().getDimensionPixelSize(R.dimen.spacing_m));
            activityTarget = a.getString(R.styleable.STItem2_STItem2_activityTarget);
            rectAroundCorners = a.getInteger(R.styleable.STItem2_STItem2_rectAroundCorners, 0);
        }
        ...
        // layoutPosition值采用布局
        if (isLayout) {
            if (rectAroundCorners != 0) {
                setBackground(ContextCompat.getDrawable(context, R.drawable.transition_st_item_islayout_lt_lb));
            } else {
                setBackground(ContextCompat.getDrawable(context, R.drawable.transition_st_item_islayout));
            }
        } else {
            setStyle();
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (isLayout) {
            ...
            if (params instanceof LinearLayout.LayoutParams) {
                ((LinearLayout.LayoutParams) params).setMargins(0, defaultMarginTop, 0, 0);
            }
    ...
    protected void setStyle() {
        ...
        switch (layoutPosition) {
            case POSITION_TOP:
                drawableId = R.drawable.transition_st_item_top;
                break;
            case POSITION_BOTTOM:
                drawableId = R.drawable.transition_st_item_bottom;
                break;
            case POSITION_MIDDLE:
            default:
                drawableId = R.drawable.transition_st_item;
                break;
        }
    ...
    protected void setErrorStyle() {
        ...
        switch (layoutPosition) {
            case POSITION_TOP:
                drawableId = R.drawable.shape_settings_item_top_error_stroke;
                break;
            case POSITION_BOTTOM:
                drawableId = R.drawable.shape_settings_item_bottom_error_stroke;
                break;
            case POSITION_MIDDLE:
            default:
                drawableId = R.drawable.shape_settings_item_error_stroke;
                break;
        }
        setBackground(ContextCompat.getDrawable(mContext, drawableId));
    }
    ...
    public void setDesc(String desc) {
        if (descTextView != null) descTextView.setText(desc);
    }
