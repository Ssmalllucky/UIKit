...
/**
 * 描述 : RecyclerView GridSpaceItemDecoration2 等间距。
 * <p>
 * 等间距需满足两个条件：
...
public class GridSpaceItemDecoration2 extends RecyclerView.ItemDecoration {
     ...
     */
    public GridSpaceItemDecoration2(int spanCount, int rowSpacing, int columnSpacing) {
    ...
    public void getItemOffsets(@NonNull Rect outRect, @NonNull View view, @NonNull RecyclerView parent, @NonNull RecyclerView.State state) {
        ...
        if(column == 0){
            outRect.right = mColumnSpacing ; // 列间距 - (column + 1) * (列间距 * (1f /列数))
        } else if(column == 1){
            if(position != 10){
                outRect.right = mColumnSpacing; // 列间距 - (column + 1) * (列间距 * (1f /列数))
            }
